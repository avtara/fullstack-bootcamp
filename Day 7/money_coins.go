...
func moneyCoins(money int) []int {
	var moneyCollection = []int{1, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000}
	var res []int
	for i := len(moneyCollection) - 1; i >= 0; i-- {
		for money > moneyCollection[i] {
			money -= moneyCollection[i]
			res = append(res,moneyCollection[i])
		}
	}
	return res
}
